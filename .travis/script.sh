...
then
	docker run -t -v "${PWD}:/libmsiecf" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libmsiecf && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libmsiecf;
